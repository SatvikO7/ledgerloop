...
python run.py                          # reconcile, then open the dashboard
```

`run.py` is a **launcher, not a second pipeline**: it runs
`python -m ledgerloop.cli demo --no-ui` and then
`python -m streamlit run src/ledgerloop/ui/app.py` — both still work on their own, and
both are what a developer should use. It exists so a reviewer does not have to know
either. **No API key is required**; the demo path is deterministic and offline, and the
